...
namespace DangerMap.JWT
{
    ...
    public class JwtAuthenticationManager : IJwtAuthenticationManger
    {
        ...
        {
            var user = database.AccountProfiles.Where(x => x.AccountId == idPassword.AccountId && x.Password == AccountProfileService.ToMD5(idPassword.Password)).SingleOrDefault();
            if (user == null)
            {
                return null;
            }
        ...
        public AuthenticationResponse Refresh(AuthenticationResponse refreshCred)
        {
            ...
            var accountID = pricipal.Claims.Where(x => x.Type == "ID").Select(x => x.Value).SingleOrDefault();
